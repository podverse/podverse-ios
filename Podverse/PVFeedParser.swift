...
class PVFeedParser {
    ...
    func parsePodcastFeed(feedUrlString:String) {
        ...
        guard feedUrlString.count > 0, let url = URL(string: feedUrlString) else { return }
    ...
    func parseResultRSS(feed:RSSFeed) {
        ...
        let podcast = CoreDataHelper.retrieveExistingOrCreateNewPodcast(feedUrlString: feedUrl, moc: privateMoc)
